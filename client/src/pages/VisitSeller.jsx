...
const VisitSeller = () => {
  ...
  const { vendor_address } = useParams();
  ...
  useEffect(() => {
     var endpoint = `/api/seller-products/${vendor_address}`
    fetch(endpoint)
      .then((res) => res.json())
      .then((data) => {
        setProducts(data.products)
        setFiltered(data.products)
        setSellerName(data.products[0].seller_name)
        setLoading(false)
      })
  }, [])
  ...
  useEffect(() => {
    ...
    navigate(`/vendor/${vendor_address}?${params.toString()}`, { replace: true })
  ...
  return (
    <>
        ...
        <h2 className='text-center mt-3 text-4xl font-bold mb-4 italic'>{sellerName}</h2>
        {!loadingAddress && walletAddress && walletAddress === vendor_address ?
        <Button className={"block mx-auto mt-10"} onClick={() => navigate("/add")}>Add Product</Button> : null}
