...
import TransactionSellerCard from "@/components/TransactionSellerCard"
...
function TransactionsBuyer() {
  ...
  useEffect(() => {
    ...
    const fetchTransactions = async () => {
      try {
        ...
        setTransactions(data.seller)
        setFiltered(data.seller)
      } catch (err) {
        console.error(err)
      } finally {
        setLoading(false)
  ...
  useEffect(() => {
    ...
    navigate(`/transactions-seller?${params.toString()}`, { replace: true })
    ...
    if (debouncedSearchTerm.trim()) {
      ...
      data = data.filter(txn =>
        txn.buyer?.toLowerCase().includes(term) ||
          ...
          <div className="bg-card border border-border rounded-xl p-6 mb-12">
            ...
            <div className="flex flex-wrap gap-4 mb-4">
              <div className="relative flex-1 min-w-[250px]">
                ...
                <Input
                  className="pl-10"
                  placeholder="Search by buyer address, product title, transaction ID..."
                  value={searchTerm}
